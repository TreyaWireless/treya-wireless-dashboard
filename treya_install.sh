...
echo ""
echo "[STEP 4] Setting up MariaDB Database and User..."
...
# Set custom Web Login: Username = treya, Password = redhat
mysql -u${DB_USER} -p${DB_PASS} ${DB_NAME} -e "UPDATE users SET username='${WEB_USER}', passwd=MD5('${WEB_PASS}') WHERE userid=1 OR username='Admin';" 2>/dev/null || true
...
echo ""
echo "[STEP 7] Configuring Treya Server Password..."
CONF_FILE=""
...
if [ -n "$CONF_FILE" ]; then
    ...
    grep -q "^DBPassword=" "$CONF_FILE" || echo "DBPassword=${DB_PASS}" >> "$CONF_FILE"
fi
