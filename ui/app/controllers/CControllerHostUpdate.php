...
class CControllerHostUpdate extends CControllerHostUpdateGeneral {
	...
	public static function getValidationRules(): array {
		...
		return ['object', 'api_uniq' => $api_uniq, 'fields' => [
			...
			'monitored_by' => ['db hosts.monitored_by', 'in' => [ZBX_MONITORED_BY_SERVER, ZBX_MONITORED_BY_PROXY, ZBX_MONITORED_BY_PROXY_GROUP, ZBX_MONITORED_BY_API]],
			...
			if ($monitored_by == ZBX_MONITORED_BY_API) {
				...
				if (!$api_macro_exists) {
					$raw_macros[] = [
						...
						'value' => '1',
						'description' => 'Flag to indicate monitoring via API'
					];
				}
			}
			else {
				foreach ($raw_macros as $idx => $macro) {
					if (isset($macro['macro']) && $macro['macro'] === '{$MONITORED_BY_API}') {
						unset($raw_macros[$idx]);
					}
				}
				$raw_macros = array_values($raw_macros);
			}
